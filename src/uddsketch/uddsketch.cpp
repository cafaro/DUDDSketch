...
double UDDSketch::get_bucket_count(long key) {
    auto bucket = this->store.find(key);
    return (bucket != this->store.end()? bucket->second : -1);
}

double UDDSketch::get_zero_bucket_count() {
    return this->zero_bucket;
}

double UDDSketch::get_sketch_count(){
    double tcount = 0;
...
void UDDSketch::sketch_restore_from_p2p_average(double p_estimate){
    for (auto &b : this->store) {
        b.second = round(b.second * p_estimate);
    }
}
...
void UDDSketch::collapse(int n) {
    if (n <= 0) return;
    std::unordered_map<long, double> new_store;
...
double UDDSketch::get_quantile(double q) {
    double rank = q * get_sketch_count();
    if (rank < this->zero_bucket) {
        return 0.0;
    }
    std::map<long, double> ordered_store;
    ...
    double count = this->zero_bucket + it->second;
    long bkey = it->first;
...
double UDDSketch::get_quantile_int(double q) {
    double rank = floor(q * (get_sketch_count()-1));
    if (rank < this->zero_bucket) {
        return 0.0;
    }
    std::map<long, double> ordered_store;
...
std::vector<uint8_t> UDDSketch::serialize() {
    ...
    bptr += copy_to_mem<double>(this->zero_bucket, bptr);
    ...
    for (const auto &b : this->store) {
        ...
        bptr += copy_to_mem<double>(b.second, bptr);
    }
    return bytes;
}
...
UDDSketch UDDSketch::deserialize(void *bytes, std::size_t size){
    ...
    bptr += copy_from_mem<double>(bptr, sketch.zero_bucket);
    size_t store_size = 0;
    ...
    for (int i = 0; i < store_size; i++) {
        ...
        bptr += copy_from_mem<double>(bptr, bcount);
...
void UDDSketch::mergeAndMean(UDDSketch& other ) {
...
std::map<long, double> UDDSketch::get_store_snap() {
    std::map<long, double> ordered_store;
